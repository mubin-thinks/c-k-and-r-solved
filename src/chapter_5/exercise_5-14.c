...
void quick_sort(void *v[], int l, int r, int (*comp)(void *a, void *b), int reverse);
...
void quick_sort(void *v[], int l, int r, int (*comp)(void *a, void *b), int reverse) {
        ...
        int comp_result;
        for (int i = l + 1; i <= r; i++) {
                comp_result = reverse ? (*comp)(v[i], v[l]) > 0 : (*comp)(v[i], v[l]) < 0;
                if (comp_result) swap(v, ++last, i);
        }
        swap(v, l, last);
        quick_sort(v, l, last - 1, comp, reverse);
        quick_sort(v, last + 1, r, comp, reverse);
}
